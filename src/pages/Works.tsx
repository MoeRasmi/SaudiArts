  ...
  return (
    <div className="bg-[#F8F6F2] min-h-screen text-[#1C1A17] font-sans overflow-hidden">
          ...
          {featured.map((item, idx) => (
            ...
            >
              ...
              <div className="space-y-8 max-w-lg">
                ...
                {item.dimensions && (
                  <div className="text-xs uppercase tracking-widest text-[#9E9790]">
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-12 mb-12">
        <div className="flex flex-col md:flex-row justify-between items-center border-b border-[#D6CFC0] pb-6 text-xs uppercase tracking-widest text-[#6B6560]">
          ...
          <div className="flex space-x-6 items-center w-full md:w-auto justify-end">
            ...
            <select value={sort} onChange={e => setSort(e.target.value)} className="bg-transparent outline-none cursor-pointer hover:text-[#1C1A17] hover:bg-transparent !p-0">
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-12 pb-32">
        {loading ? (
          <div className="text-center py-32 text-xs uppercase tracking-widest text-[#8C6A3B]">Loading Collection...</div>
        ) : (
              ...
              {filtered.map((item, idx) => (
                ...
                >
                  <Link to={`/artwork/${item.slug || item.id}`} className="group block relative aspect-[4/5] bg-[#EDE8DC] overflow-hidden">
                    ...
                    {/* Dark Overlay */}
                    <div className="absolute inset-0 bg-[#1C1A17]/0 group-hover:bg-[#1C1A17]/60 transition-colors duration-500 flex flex-col justify-end p-8 pb-10 opacity-0 group-hover:opacity-100">
                      <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500 delay-75 space-y-3">
                        ...
                        <div className="text-[#D4B06A] text-[10px] uppercase tracking-widest border-b border-[#D4B06A]/30 pb-3">
                          {item.category || item.categoryId} • {item.period}
                        </div>
                        ...
                        <div className="inline-block mt-4 text-[9px] uppercase tracking-widest px-2 py-1 border border-[#D4B06A]/30 text-[#D4B06A]">
                          Private Collection
                        </div>
                      </div>
                    </div>
